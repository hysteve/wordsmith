...
router.get('/domains', async (req, res) => {
  const { domain, categories, suggestions = 10, maxTries = 50, timeout = 30000, outputPath = './output/', useSynonyms = false, useSameStart = false, useTriggers = false, useFollowers = false, useFollowSuffix = false } = req.query;

  let categoriesList = categories ? categories.split(',') : [];
  if (!categoriesList.length) {
    categoriesList = await promptForCategories();
  }
